...
###
# run hdfs to teradata export
#
# Arguments:
#   teradata_connector_file
#     - teradata connector jar file path
#   teradata_username
#     - teradata login user
#   teradata_password_file
#     - teradata login user password
#   teradata_server
#     - name/ip of the teradata server
#   teradata_database
#     - teradata table database name
#   teradata_table
#     - teradata table name
#   teradata_hdfs_dir
#     - hdfs data dirctory
#
function fn_hdfs_to_teradata_export() {

  teradata_connector_file="$1"
  teradata_username="$2"
  teradata_password_file="$3"
  teradata_server="$4"
  teradata_database="$5"
  teradata_table="$6"
  teradata_hdfs_dir="$7"
  ...
  fn_assert_variable_is_set "teradata_connector_file" "${teradata_connector_file}"
  fn_assert_file_exists "${teradata_connector_file}" "${BOOLEAN_TRUE}"
  fn_assert_variable_is_set "teradata_username" "${teradata_username}"
  fn_assert_variable_is_set "teradata_password_file" "${teradata_password_file}"
  fn_assert_variable_is_set "teradata_server" "${teradata_server}"
  fn_assert_variable_is_set "teradata_database" "${teradata_database}"
  fn_assert_variable_is_set "teradata_table" "${teradata_table}"
  fn_assert_variable_is_set "teradata_hdfs_dir" "${teradata_hdfs_dir}"
  fn_assert_variable_is_set "number_of_mappers" "${number_of_mappers}"
  fn_assert_variable_is_set "separator" "${separator}"

  teradata_connect=jdbc:teradata://${teradata_server}/DATABASE=${teradata_database},CHARSET=UTF16

  hadoop jar "${teradata_connector_file}" \
    com.teradata.connector.common.tool.ConnectorExportTool \
    -D tdch.num.mappers=${number_of_mappers} \
    -url ${teradata_connect} \
    -username ${teradata_username} \
    -password `cat ${teradata_password_file}` \
    -jobtype hdfs \
    -sourcepaths "${teradata_hdfs_dir}" \
    -separator "${separator}" \
    -targettable "${teradata_table}" \
    -forcestage true \
    -stagedatabase ${teradata_database} \
    -stagetablename "${teradata_table}_temporary" \
    -method internal.fastload

  exit_code=$?
  success_message="Successfully exported data to teradata table ${teradata_database}.${teradata_table}"
  failure_message="Failed to export data to teradata table ${teradata_database}.${teradata_table}"
  fn_handle_exit_code "${exit_code}" "${success_message}" "${failure_message}" "${fail_on_error}"
...
#   refresh_url
#     - refresh url for storage account
#   client_id
#     - client id for the storage account
#   credential_file
#     - credential_file for the storage account
#   sink_path
#     - path of the sink
#   required_jars
#     - jars required for the export
#
function fn_hdfs_26_to_azure_export() {

  source_path="$1"

  fn_assert_variable_is_set "source_path" "${source_path}"

  max_num_mappers="$2"

  fn_assert_variable_is_set "max_num_mappers" "${max_num_mappers}"

  refresh_url="$3"

  fn_assert_variable_is_set "refresh_url" "${refresh_url}"

  client_id="$4"

  fn_assert_variable_is_set "client_id" "${client_id}"

  credential_file="$5"

  fn_assert_variable_is_set "credential_file" "${credential_file}"

  sink_path="$6"

  fn_assert_variable_is_set "sink_path" "${sink_path}"

  required_jars="$7"

  fn_assert_variable_is_set "required_jars" "${required_jars}"

  for filename in `hadoop fs -ls $source_path | \
                    awk '{print $NF}' | \
                    tr '\n' ' '` ; do

        if [[ $filename != *".completed" && $filename != "items" ]]; then

            fn_log_info "exporting $filename to azure"
            hadoop distcp \
             -D fs.adl.impl=org.apache.hadoop.fs.adl.AdlFileSystem \
             -D fs.AbstractFileSystem.adl.impl=org.apache.hadoop.fs.adl.Adl \
             -D mapreduce.framework.name=local \
             -D mapreduce.local.map.tasks.maximum=10 \
             -D dfs.adls.oauth2.access.token.provider.type=ClientCredential \
             -D dfs.adls.oauth2.refresh.url=${refresh_url} \
             -D dfs.adls.oauth2.client.id=${client_id} \
             -D dfs.adls.oauth2.credential=`cat ${credential_file}` \
             --libjars ${required_jars} \
             ${source_path} ${sink_path}
             exit_code=$?
               success_message="Successfully exported data to azure"
               failure_message="Failed to export data to azure"
               fn_handle_exit_code "${exit_code}" "${success_message}" "${failure_message}" "${fail_on_error}"

             fn_move_file_hadoop $filename $filename.completed
         fi
...
function fn_hdfs_26_to_azure_export_gen2() {
  ...
  for filename in `hadoop fs -ls $source_path | \
        ...
        if [[ $filename != *".completed" && $filename != "items" ]]; then
            ...
            fn_log_info "exporting $filename to azure"
            hadoop distcp \
             -D fs.abfs.impl=org.apache.hadoop.fs.azurebfs.AzureBlobFileSystem \
             -D fs.AbstractFileSystem.abfs.impl=org.apache.hadoop.fs.azurebfs.Abfs \
             -D fs.azure.account.auth.type.absitdsdevwus001.dfs.core.windows.net=OAuth \
             -D fs.azure.account.oauth2.client.endpoint=https://login.microsoftonline.com/b7f604a0-00a9-4188-9248-42f3a5aac2e9/oauth2/token \
             -D fs.azure.account.oauth2.client.id=4b812f96-1165-4b7f-bc5d-91de6c86e79b \
             -D fs.azure.account.oauth2.client.secret=+TIL4bT6vbQEndCtXNukb1Tz654GZi1QfP2yhCS5iZ0= \
             -D fs.azure.account.oauth.provider.type.absitdsdevwus001.dfs.core.windows.net=org.apache.hadoop.fs.azurebfs.oauth2.ClientCredsTokenProvider \
             --libjars ${required_jars} \
             ${source_path} ${sink_path}
             exit_code=$?
               success_message="Successfully exported data to azure"
...
function fn_move_file_hadoop(){
  ...
  fn_assert_variable_is_set "old_path" "${old_path}"

  fn_assert_variable_is_set "new_path" "${new_path}"

  hdfs dfs -mv  "${old_path}" "${new_path}"

  exit_code=`fn_get_exit_code $?`

  success_message="Successfully moved file from ${old_path} to ${new_path}"

  failure_message="Failed to move file from ${old_path} ${new_path}"
...
#Arguments:
#  source_path
#  - path of source directory on hdfs
#  target_path
#  - path of target directory on hdfs
function fn_move_files_on_hdfs(){
  ...
  success_message="Successfully moved files from ${source_path} to ${target_path}"

  failure_message="Failed to move files from ${source_path} to ${target_path}"
