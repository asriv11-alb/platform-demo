...
MSCK REPAIR TABLE increment_retail_customer_email
;

INSERT OVERWRITE TABLE ${DB_WORK}.temp_retail_customer_email
  SELECT T1.*
    FROM ${DB_WORK}.current_retail_customer_email T1
    LEFT OUTER JOIN ${DB_WORK}.increment_retail_customer_email T2
    ON (
 ...
 UNION
  SELECT
   ...
   BY employee_sk ,document_alternatedocumentid ) AS max_creationtm
            FROM ${DB_WORK}.increment_retail_customer_email) AS SUB
